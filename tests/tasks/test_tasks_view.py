...
def test_tasks_list_view(db, django_db_setup, client, task_model_test_fixtures, user_model_test_fixtures):
    ...
    assert next(filter(lambda template: template.name == 'tasks/list.html', response.templates), False)
    # Check QuerySet
...
def test_tasks_filter_by_status(db, django_db_setup, client, task_model_test_fixtures, user_model_test_fixtures):
    user_data = user_model_test_fixtures['login']['user1']['auth_data'].copy()
    client.login(
        username=user_data['username'],
        password=user_data['password'],
...
def test_tasks_filter_by_executor(db, django_db_setup, client, task_model_test_fixtures, user_model_test_fixtures):
    user_data = user_model_test_fixtures['login']['user1']['auth_data'].copy()
    client.login(
        username=user_data['username'],
        password=user_data['password'],
...
def test_tasks_filter_by_label(db, django_db_setup, client, task_model_test_fixtures, user_model_test_fixtures):
    user_data = user_model_test_fixtures['login']['user1']['auth_data'].copy()
    client.login(
        username=user_data['username'],
        password=user_data['password'],
...
def test_tasks_filter_own_tasks(db, django_db_setup, client, task_model_test_fixtures, user_model_test_fixtures):
    user_data = user_model_test_fixtures['login']['user1']['auth_data'].copy()
    client.login(
        username=user_data['username'],
        password=user_data['password'],
...
def test_details_view(db, django_db_setup, client, task_model_test_fixtures, user_model_test_fixtures):
    user_data = user_model_test_fixtures['login']['user1']['auth_data'].copy()
    client.login(
        username=user_data['username'],
        password=user_data['password'],
    ...
    assert response.status_code == 200
    assert next(filter(lambda template: template.name == 'tasks/details.html', response.templates), False)
    assert task1.name in str(response.content.decode('utf-8'))
...
def test_tasks_create_view(db, django_db_setup, client, task_model_test_fixtures, user_model_test_fixtures):
    user_data = user_model_test_fixtures['login']['user1']['auth_data'].copy()
    client.login(
        username=user_data['username'],
        password=user_data['password'],
...
def test_tasks_update_view(db, django_db_setup, client, task_model_test_fixtures, user_model_test_fixtures):
    user_data = user_model_test_fixtures['login']['user1']['auth_data'].copy()
    client.login(
        username=user_data['username'],
        password=user_data['password'],
...
def test_tasks_delete_view(db, django_db_setup, client, task_model_test_fixtures, user_model_test_fixtures):
    user_data = user_model_test_fixtures['login']['user1']['auth_data'].copy()
    client.login(
        username=user_data['username'],
        password=user_data['password'],
    ...
    assert response.status_code == 200
    assert next(filter(lambda template: template.name == 'tasks/delete.html', response.templates), False)


def test_tasks_delete_view_different_user(db, django_db_setup, client, task_model_test_fixtures, user_model_test_fixtures):
